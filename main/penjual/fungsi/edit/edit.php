...
if (!empty($_SESSION['admin'])) {
	...
	if (!empty($_GET['pengaturan'])) {
		...
		$data[] = $pemilik;
		$data[] = $id;
		$sql = 'UPDATE toko SET nama_toko=?, alamat_toko=?, tlp=?, nama_pemilik=? WHERE id_member = ?';
	...
	if (!empty($_GET['gmbr'])) {
		...
		if ($_FILES['fotot']["error"] > 0) {
			$output['error'] = "Error in File";
		} elseif (!in_array($_FILES['fotot']["type"], $allowedImageType)) {
		...
		} elseif (round($_FILES['fotot']["size"] / 1024) > 4096) {
			echo "WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB";
			echo "<font face='Verdana' size='2' ><BR><BR><BR>
					<a href='../../index.php?page=barang'>Back to upform</a><BR>";
		} else {
			$target_path = '../../assets/img/barang/';
			$target_path = $target_path . basename($_FILES['fotot']['name']);
			if (file_exists("$target_path")) {
				...
				echo "<font face='Verdana' size='2' ><BR><BR><BR>
					<a href='../../index.php?page=barang'>Back to upform</a><BR>";
			} elseif (move_uploaded_file($_FILES['fotot']['tmp_name'], $target_path)) {
				//post foto lama
				$fotot2 = $_POST['fotot2'];
				//remove foto di direktori
				unlink('../../assets/img/barang/' . $fotot2 . '');
				//input foto
				$id = $_POST['id'];
				$data[] = $_FILES['fotot']['name'];
				$data[] = $id;
				$sql = 'UPDATE barang SET gmbr=?  WHERE barang.id_barang=?';
				$row = $config->prepare($sql);
				$row->execute($data);
